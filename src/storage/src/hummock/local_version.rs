...
use super::shared_buffer::shared_buffer::SharedBuffer;
...
impl LocalVerion {
    ...
    pub fn get_shared_buffer(
        &self,
        epoch: HummockEpoch,
    ) -> Option<&Arc<RwLock<SharedBuffer>>> {
    ...
    pub fn new_shared_buffer(
        ...
        let res = self
            .shared_buffer
            .insert(epoch, new_shared_buffer.clone());
    ...
    pub fn read_version(&self) -> ReadVersion {
        ...
        ReadVersion {
            shared_buffer: self
                .shared_buffer
                .range(smallest_uncommitted_epoch..)
                .rev() // Important: order by epoch descendingly
                .map(|e| e.1.read_arc())
                .collect(),
            uncommitted_ssts: self
                .uncommitted_ssts
                .range(smallest_uncommitted_epoch..)
                .rev() // Important: order by epoch descendingly
